...
from pyspark.sql.functions import explode, col, lit, reduce, input_file_name, regexp_extract, current_timestamp, to_timestamp, to_date, row_number, desc, concat, pmod, hash, from_utc_timestamp
...
REALTIME_SPEC_PATH = "gtfs-realtime.desc"  # generated using protoc and gtfs-realtime.proto (https://gtfs.org/documentation/realtime/proto/)
...
def to_eastern(col_name):
    ...
    return from_utc_timestamp(
        to_timestamp(col(col_name)),  # epoch int -> UTC timestamp
        "America/New_York"
    )
...
def extract_vehicle_position_cols(df):
    ...
    vehicle_df = (df
        .select(
            ...
            col("feed.header.timestamp").alias("snapshot_timestamp"),  # when the API was called
        ...
        .select(
            ...
            col("entity.vehicle.multi_carriage_details").alias("multi_carriage_details"),
...
def load_data_from_realtime_s3_to_df(spark, feed_type, service_date):
    ...
    if feed_type == TRIP_UPDATE:   
        return load_trip_updates_data_from_realtime_s3_to_df(spark, feed_type, service_date)
...
def write_raw_df_to_snowflake(df, table_name, service_date, hour=None):
    ...
    delete_query = f"DELETE FROM FINAL_PROJECT_RAW.{table_name} WHERE service_date = '{service_date}'"

    if hour is not None:
        delete_query = f"DELETE FROM FINAL_PROJECT_RAW.{table_name} WHERE service_date = '{service_date}' AND hour = {hour}"
    
    sfOptions = {
      "sfURL": "sfedu02-unb02139.snowflakecomputing.com",
      "sfUser": "LEMMING",
      "sfDatabase": "LEMMING_DB",
      "sfSchema": "FINAL_PROJECT_RAW",
      "sfWarehouse": "LEMMING_WH",
      "pem_private_key": f"{pkb_string}",
      "preactions": delete_query  # idempotency guard
    }

    df.write.format("net.snowflake.spark.snowflake").options(**sfOptions).option("dbtable", table_name).mode("append").save()
...
if __name__ == "__main__":
    ...
    rt_feed_option = VEHICLE_POSITION
    ...
    if raw_rt_df is not None: # (feed option trip updates returns None as of now)
        write_raw_df_to_snowflake(raw_rt_df, f"RAW_{rt_feed_option.upper()}", service_date)
